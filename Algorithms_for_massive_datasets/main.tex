...
Implement a system that classifies Cypher queries by difficulty level (node level, 1 hop, 2 hop, 3 hop, hard level), training on a large dataset. (Definitions of Cypher queries and difficulty levels are given in the appendix.)
...
\item Approach 3
Train CodeBERT on Text2Cypherr, then fine-tune on the curated dataset, evaluate as above.
...
I fine-tuned CodeBERT on the large Text2Cypher dataset, with an optional subsampling (Training is performed in mini‑batches on GPU step to control runtime and it can scale to the full dataset when subsampling is disabled (I used 50\% of the data). For this purpose, I first loaded the required libraries and datasets, and normalized the labels to ensure consistency across the different data sources. I then defined the main training configuration, including the hyperparameters, and applied stratified subsampling because the dataset is highly imbalanced. This was done to preserve class proportions and reduce the risk of excluding rare classes during sampling. The train, validation, and test splits were also created using stratification in order to maintain the same class distribution across all subsets.
...
Training is performed in mini‑batches on GPU. 
